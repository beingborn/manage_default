    ...
    (t.prototype.beforeDatasetsUpdate = function (t) {
          ...
          if (0 !== h.circumference || this.showZero) {
            ...
            if (
              ("function" == typeof this.render &&
                "object" ==
                  typeof (r = this.render({
                    label: i.config.data.labels[a],
                    value: t.data[a],
                    percentage: f,
                    dataset: t,
                    index: a,
            ...
            if (
              ...
              "border" === this.position
                ? (u = (h.outerRadius - d) / 2 + d)
                : "outside" === this.position &&
                  (this.segment && (g = 20), (u = h.outerRadius - d + d + g)),
                (c = { x: h.x + Math.cos(x) * u, y: h.y + Math.sin(x) * u }),
                "outside" === this.position &&
                  (c.x < h.x ? (c.x -= g) : (c.x += g),
                  (p = h.outerRadius + g));
            ...
            if (
              ("function" == typeof y
                ...
                : "string" != typeof y &&
                  (y = y[a] || this.options.defaultFontColor),
              this.arc)
            )
              p || (p = (d + h.outerRadius) / 2),
                (e.fillStyle = y),
                (e.textBaseline = "middle"),
                this.drawArcText(r, p, h, this.overlap);
            else {
              var m,
                b = this.measureText(r),
                v = c.x - b.width / 2,
                w = c.x + b.width / 2,
              ...
              if (this.overlap) m = !0;
                ...
                if (this.segment) {
                  start = {
                    x: h.x + Math.cos(x) * h.outerRadius,
                    y: h.y + Math.sin(x) * h.outerRadius,
                  };
                  var C = u - 10;
                  if (
                    ((end = {
                    ...
                    e.beginPath(),
                    e.moveTo(start.x, start.y),
                    "auto" == this.segmentColor)
                  )
                    var A = h.backgroundColor;
                  else A = this.segmentColor;
                  (e.strokeStyle = A),
                    (e.lineWidth = 1),
                    e.lineTo(end.x, end.y),
                    start.x < end.x
                      ? ((c.x = c.x + 10), (end.x = c.x - b.width / 2 - 1))
                      : ((c.x = c.x - 10), (end.x = c.x + b.width / 2 + 1)),
                    (v = c.x - b.width / 2),
                    (w = c.x + b.width / 2),
                    (S = c.y - this.fontSize / 2),
      ...
      (t.prototype.parseOptions = function (t) {
        ...
        return (
          ...
          (this.segment = e.segment || !1),
          (this.segmentColor = e.segmentColor || "auto"),
          !0)
        );
      }),
